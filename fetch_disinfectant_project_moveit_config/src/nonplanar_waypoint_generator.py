...
########################## Test Case 1: Fixed height ###########################
        z_index = self.cloud_z.index(max(self.cloud_z))
        for i in range(len(px)):
            ...
            p.position.z = self.cloud_z[z_index] + self.offset
            p.orientation.x = 0
            p.orientation.y = .707
            p.orientation.z = 0
            p.orientation.w = .707
...
################ Test Case 3: Offset distance from point normal ################
    ...
    def rotation_matrix(self,alpha, gamma, beta):
        ...
        # print(R_y)
        # print(R_Z)
        # print(np.linalg.multi_dot([R_z,R_y,R_Z]))
        # print("")

        return np.linalg.multi_dot([R_z,R_y,R_Z])
