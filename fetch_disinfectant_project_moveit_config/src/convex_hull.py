...
class Convex_hull:
    def __init__(self):
        ...
        # Initialize a list for the Points of each interactive marker.
        self.clicked_points_3D = []

        # Initialize id of interactive markers.
        self.id = 0
    ...
    def makeInteractiveMarker(self, msg):
        ...
        # update the interactive marker id for next clicked point.
        self.id += 1
    ...
    def convex_hull_function(self):
        # create an empty list to store the x and y positions of the clicked points
        # computing the convex hull relies only on 2-D points.
        points_2D = []
        for i in range(len(self.clicked_points_3D)):
            points_2D.append([self.clicked_points_3D[i][0], self.clicked_points_3D[i][1]])

        # create an empty list to store the 3-D data of each convex point. This
        # will be published and visualized in rviz.
        convex_points = []

        # Run the ConvexHull function from scipy.spatial and store data
        # to convex_points with a forloop.
    ...
    # processFeedback gets called when an interactive marker's position is changed
